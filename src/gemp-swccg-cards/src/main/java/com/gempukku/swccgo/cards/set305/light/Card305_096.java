 ...
 */
public class Card305_096 extends AbstractSite {
    public Card305_096() {
        super(Side.LIGHT, Title.Senate_Hallway, Title.Quermia, Uniqueness.UNIQUE, ExpansionSet.ABT, Rarity.U);
        setLocationDarkSideGameText("If your battle droid present, Force drain +1 here.");
        setLocationLightSideGameText("Once per game may take Hidden Corridors into hand from Reserve Deck; reshuffle.");
    ...
    protected List<TopLevelGameTextAction> getGameTextLightSideTopLevelActions(String playerOnLightSideOfLocation, SwccgGame game, final PhysicalCard self, int gameTextSourceCardId) {
        GameTextActionId gameTextActionId = GameTextActionId.SENATE_HALLWAY__UPLOAD_HIDDEN_CORRIDORS;
        ...
        if (GameConditions.isOncePerGame(game, self, gameTextActionId)
            ...
            action.setText("Take Hidden Corridors into hand from Reserve Deck");
            // Update usage limit(s)
            action.appendUsage(
                    new OncePerGameEffect(action));
            // Perform result(s)
            action.appendEffect(
                    new TakeCardIntoHandFromReserveDeckEffect(action, playerOnLightSideOfLocation, Filters.Hidden_Corridors, true));
