 ...
 * Type: Interrupt
 * Subtype: Lost
 * Title: Hubris Of A Jedi
 */
public class Card305_084 extends AbstractLostInterrupt {
    public Card305_084() {
        super(Side.LIGHT, 6, "Hubris Of A Jedi", Uniqueness.UNIQUE, ExpansionSet.ABT, Rarity.PM);
        setLore("Despite all the lessons of the Jedi they still believe themselves to be invincible.");
        setGameText("If your Jedi was just defeated in lightsaber combat, you may either: Reveal a Dark Jedi's combat cards and place one in opponent's Lost Pile (return others). OR Place Teikhos Ta'var out of play and take [COU] Turel Sorenn into hand from Lost Pile or Reserve Deck; reshuffle.");
        addIcons(Icon.ABT);
    }
    ...
    protected List<PlayInterruptAction> getGameTextOptionalAfterActions(final String playerId, final SwccgGame game, EffectResult effectResult, final PhysicalCard self) {
        ...
        if (TriggerConditions.wonLightsaberCombatAgainst(game, effectResult, Filters.any, Filters.and(Filters.your(self), Filters.Jedi))) {
            ...
            final GameTextActionId gameTextActionId = GameTextActionId.HUBRIS_OF_A_JEDI__TAKE_TUREL_INTO_HAND;

            if (GameConditions.canTarget(game, self, TargetingReason.TO_BE_PLACED_OUT_OF_PLAY, Filters.Teikhos)
                ...
                action.appendTargeting(new TargetCardOnTableEffect(action, playerId, "Choose Teikhos Ta'var to place out of play", TargetingReason.TO_BE_PLACED_OUT_OF_PLAY, Filters.Teikhos) {
                    @Override
                    protected void cardTargeted(final int targetGroupId, final PhysicalCard targetedCard) {
                        action.allowResponses("Take [COU] Turel Sorenn into hand from Lost Pile or Reserve Deck", new RespondablePlayCardEffect(action) {
                            @Override
                            protected void performActionResults(Action targetingAction) {
                                ...
                                if (canSearchReserveDeck
                                    ...
                                    action.appendEffect(new PlayoutDecisionEffect(action, playerId, new YesNoDecision("You will search your Lost Pile. Do you also want to search your Reserve Deck?") {
                                        ...
                                        protected void yes() {
                                            ...
                                            action.appendEffect(new ChooseCardsFromMultiplePilesEffect(action, playerId, zones, playerId,1, 1, 1, false, false, Filters.and(Icon.COU, Filters.Turel)) {
                                                @Override
                                                protected void cardsSelected(SwccgGame game, Collection<PhysicalCard> selectedCards) {
                                                    if (selectedCards.size()>0) {
                                                        action.appendEffect(new TakeOneCardIntoHandFromOffTableEffect(action, playerId, selectedCards.iterator().next(), "Take Turel Sorenn into hand") {
                                                            @Override
                                        ...
                                        protected void no() {
                                            action.appendEffect(
                                                    new TakeCardIntoHandFromLostPileEffect(action, playerId, Filters.and(Icon.COU, Filters.Turel), false));
                                        }
                                    }));
                                } else if (canSearchReserveDeck
                                        && !canSearchLostPile) {
                                    action.appendEffect(
                                            new TakeCardIntoHandFromReserveDeckEffect(action, playerId, Filters.and(Icon.COU, Filters.Turel), true));
