 ...
 * Title: 7VD-009
 */
public class Card305_114 extends AbstractDroid {
    public Card305_114() {
        super(Side.DARK, 3, 2, 3, 4, "7VD-009", Uniqueness.UNIQUE, ExpansionSet.ABT, Rarity.U);
        setArmor(3);
        setLore("Officer battle droid who acted as a unit leader in the Vauzem Dominion's attack against the Quermians.");
        setGameText("Adds 2 to power of anything he pilots. May add or subtract one from a just drawn Energy Shell weapon destiny draw here. While at Quermia Forest, twice per game may take Open Fire! or Cease Fire! into hand from Reserve Deck; reshuffle.");
    ...
    protected List<TopLevelGameTextAction> getGameTextTopLevelActions(final String playerId, final SwccgGame game, final PhysicalCard self, int gameTextSourceCardId) {
        GameTextActionId gameTextActionId = GameTextActionId._7VD009__UPLOAD_OPEN_FIRE_OR_CEASE_FIRE;

        // Check condition(s)
        if (GameConditions.isTwicePerGame(game, self, gameTextActionId)
                && GameConditions.isAtLocation(game, self, Filters.Quermia_Forest)
