 ...
 * Title: Kurotsubaki
 */
public class Card305_053 extends AbstractCharacterWeapon {
    public Card305_053() {
        super(Side.LIGHT, 1, "Kurotsubaki", Uniqueness.UNIQUE, ExpansionSet.ABT, Rarity.PM);
        setAsHorizontal(true);
        setLore("Named after an exotic plant, this unique glaive style hilt and blade is wielded by Mihoshi Keibatsu. Crafted from Beskar the hilt is resistant to lightsabers leading to Mihoshi's unique combat style.");
        setGameText("Deploy on Mihoshi. Adds 1 to Mihoshi's lightsaber combat total. May lose 1 Force to add 2 to Force drain where present. Twice per battle, may target a character. Draw two destiny. Target hit, and its forfeit = 0, if total > defense value. May not be stolen.");
        addPersona(Persona.MIHOSHIS_KUROTSUBAKI);
        addIcons(Icon.ABT);
        addKeywords(Keyword.LIGHTSABER);
        setMatchingCharacterFilter(Filters.Mihoshi);
    }

    @Override
    protected Filter getGameTextValidDeployTargetFilter(SwccgGame game, PhysicalCard self, PlayCardOptionId playCardOptionId, boolean asReact) {
        return Filters.and(Filters.your(self), Filters.Mihoshi);
    }

    @Override
    protected Filter getGameTextValidToUseWeaponFilter(final SwccgGame game, final PhysicalCard self) {
        return Filters.Mihoshi;
    }

    @Override
    protected List<Modifier> getGameTextWhileActiveInPlayModifiers(SwccgGame game, final PhysicalCard self) {
        List<Modifier> modifiers = new LinkedList<Modifier>();
        modifiers.add(new LightsaberCombatTotalModifier(self, Filters.Mihoshi, 1));
