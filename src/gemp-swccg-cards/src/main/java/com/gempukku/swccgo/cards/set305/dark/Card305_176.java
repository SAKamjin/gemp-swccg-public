 ...
 * Type: Weapon
 * Subtype: Character
 * Title: Morvyn's Lightsaber
 */
public class Card305_176 extends AbstractCharacterWeapon {
    public Card305_176() {
        super(Side.DARK, 1, "Morvyn's Lightsaber", Uniqueness.UNIQUE, ExpansionSet.ABT, Rarity.R);
        setLore("After his early failures as a Sith, Morvyn constructed a new lightsaber and took his new name, Morvyn. This blade represents his dedication to his apprenticeship and the Dark Side of the Force.");
        setGameText("Deploy on Morvyn. Adds 1 to defense value and training destiny. May add 1 to Force drain where present. May target a character, creature, speeder bike, or swoop for free. Draw 2 destiny. Target hit, and its forfeit = 0 if total destiny > defense value.");
        addPersona(Persona.MORVYNS_LIGHTSABER);
        addIcons(Icon.ABT);
        addKeywords(Keyword.LIGHTSABER);
        setMatchingCharacterFilter(Filters.Morvyn);
    }

    @Override
    protected Filter getGameTextValidDeployTargetFilter(SwccgGame game, PhysicalCard self, PlayCardOptionId playCardOptionId, boolean asReact) {
        return Filters.and(Filters.your(self), Filters.Morvyn);
    }

    @Override
    protected Filter getGameTextValidToUseWeaponFilter(final SwccgGame game, final PhysicalCard self) {
        return Filters.Morvyn;
    }
