 ...
 * Type: Character
 * Subtype: Alien
 * Title: Guardian Syrena Valkar
 */
public class Card305_050 extends AbstractAlien {
    public Card305_050() {
        super(Side.LIGHT, 1, 4, 3, 5, 5, "Guardian Syrena Valkar", Uniqueness.UNIQUE, ExpansionSet.ABT, Rarity.R);
        setLore("Leader of the Knights of Allusis. Heavily involved with the Quermia campaign and fighting the invading droid army. Musician, playing the hallikset.");
        setGameText("Once per game may take Light Morning Brunch into hand from Reserve Deck; reshuffle. Once per battle, may use 1 Force to target an opponent's droid present. Both players draw destiny. If your destiny +1 > opponent's destiny, target lost.");
    ...
    protected List<TopLevelGameTextAction> getGameTextTopLevelActions(final String playerId, final SwccgGame game, final PhysicalCard self, int gameTextSourceCardId) {
        ...
        if (GameConditions.isOncePerGame(game, self, gameTextActionId)
            ...
            action.setActionMsg("Take Light Morning Brunch into hand from Reserve Deck");
            // Update usage limit(s)
            action.appendUsage(
                    new OncePerGameEffect(action));
            // Perform result(s)
            action.appendEffect(
                    new TakeCardIntoHandFromReserveDeckEffect(action, playerId, Filters.Light_Morning_Brunch, true));
            actions.add(action);
        }
