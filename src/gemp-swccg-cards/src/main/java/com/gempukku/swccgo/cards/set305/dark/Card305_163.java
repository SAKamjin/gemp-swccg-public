 ...
 * Type: Effect
 * Title: Ore Chute Airlock
 */
public class Card305_163 extends AbstractNormalEffect {
    public Card305_163() {
        super(Side.DARK, 4, PlayCardZoneOption.YOUR_SIDE_OF_TABLE, Title.Ore_Chute_Airlock, Uniqueness.UNIQUE, ExpansionSet.ABT, Rarity.U);
        setLore("The gangsters on Danktooine have a unique solution to solving problems. Ore Chutes now have a safety feature on their airlocks to prevent living beings being blown out into space.");
        setGameText("Deploy on table. Any character here may be captured or rescued by a player's starship or vehicle controlling Danktooine during that player's control phase. Character here lost if new character arrives. Effect lost if Danktooine lost. (Immune to Alter.)");
        addIcons(Icon.ABT);
    ...
    protected List<RequiredGameTextTriggerAction> getGameTextRequiredAfterTriggers(SwccgGame game, EffectResult effectResult, PhysicalCard self, int gameTextSourceCardId) {
        // Check condition(s)
        if (TriggerConditions.justRelocatedToOreChuteAirlock(game, effectResult, Filters.character)) {
            PhysicalCard newCharacter = ((RelocateToOreChuteAirlockResult) effectResult).getCard();
        ...
        // Check condition(s)
        if (TriggerConditions.justLost(game, effectResult, Filters.Danktooine_system)) {
    ...
    /**
     * Gets the top-level actions to rescue or capture the character on the Ore Chute Airlock.
        ...
        if (GameConditions.isDuringYourPhase(game, playerId, Phase.CONTROL)) {
            if (GameConditions.hasStackedCards(game, self, Filters.and(Filters.your(playerId), Filters.character))) {
                ...
                Filter starshipOrVehicleFilter = Filters.and(Filters.your(playerId), Filters.or(Filters.starship, Filters.vehicle),
                        Filters.piloted, Filters.at(Filters.and(Filters.Danktooine_system, Filters.controls(playerId))),
                ...
                if (GameConditions.canSpot(game, self, starshipOrVehicleFilter)) {
                    ...
                    action.allowResponses("Capture " + GameUtils.getCardLink(attachedCharacter),
                            new UnrespondableEffect(action) {
                                @Override
                                protected void performActionResults(Action targetingAction) {
                                    final PhysicalCard cloudCity = Filters.findFirstFromTopLocationsOnTable(game, Filters.Danktooine_system);
                                    // Perform result(s)
                                    action.appendEffect(
