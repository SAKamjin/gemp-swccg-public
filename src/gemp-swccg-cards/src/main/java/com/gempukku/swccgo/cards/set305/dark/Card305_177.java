 ...
 * Type: Character
 * Subtype: Sith
 * Title: Warrior Morvyn, Infiltrator
 */
public class Card305_177 extends AbstractSith {
    public Card305_177() {
        super(Side.DARK, 1, 4, 4, 5, 5, "Warrior Morvyn, Infiltrator", Uniqueness.UNIQUE, ExpansionSet.ABT, Rarity.R);
        setLore("Morvyn was shaped by failure, discipline, and the Dark Side. He serves the Scholae Palatinae Empire without hesitation, and most enemies who learn his name do not live long enough to repeat it.");
        setGameText("[Pilot] 3. During battle, characters 'hit' by Morvyn may not fire weapons. May [download] Morvyn's Lightsaber here. If a battle just initiated at same site, unless a vehicle here, may target a Jedi here; for remainder of battle, exclude all other characters. Immune to attrition < 4.");
    ...
    protected List<TopLevelGameTextAction> getGameTextTopLevelActions(final String playerId, final SwccgGame game, final PhysicalCard self, int gameTextSourceCardId) {
        GameTextActionId gameTextActionId = GameTextActionId.WARRIOR_MORVYN_INFILTRATOR__DOWNLOAD_MORVYNS_LIGHTSABER;

        // Check condition(s)
        if (GameConditions.canDeployCardFromReserveDeck(game, playerId, self, gameTextActionId, Persona.MORVYNS_LIGHTSABER)) {

            final TopLevelGameTextAction action = new TopLevelGameTextAction(self, gameTextSourceCardId, gameTextActionId);
            action.setText("Deploy Morvyn's Lightsaber from Reserve Deck");
            action.setActionMsg("Deploy Morvyn's Lightsaber to Morvyn's location from Reserve Deck");
            // Perform result(s)
            action.appendEffect(
                    new DeployCardToTargetFromReserveDeckEffect(action, Filters.persona(Persona.MORVYNS_LIGHTSABER), Filters.atSameLocation(self), true, true));
            return Collections.singletonList(action);
        }
        return null;
    }
    ...
    protected List<OptionalGameTextTriggerAction> getGameTextOptionalAfterTriggers(final String playerId, final SwccgGame game, EffectResult effectResult, final PhysicalCard self, int gameTextSourceCardId) {
        ...
        Filter darkJediFilter = Filters.and(Filters.Jedi, Filters.atSameLocation(self), Filters.participatingInBattle);
