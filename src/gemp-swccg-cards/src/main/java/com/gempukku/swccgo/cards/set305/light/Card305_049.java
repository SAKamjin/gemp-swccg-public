 ...
 * Title: Let's Go Right
 */
public class Card305_049 extends AbstractAdmiralsOrder {
    public Card305_049() {
        super(Side.LIGHT, "Let's Go Right", ExpansionSet.ABT, Rarity.R);
        setGameText("Non-pilot characters aboard starships are forfeit -4. Whenever a player's starship is 'hit', that player loses 1 Force (2 if starship was hit by Proton Torpedoes). At systems where you have only [COU] starships, all of those starships are immune to attrition < 4. At sites related to a system you occupy, your battle destinies may not be canceled by opponent.");
        addIcons(Icon.ABT);
    }
    ...
    protected List<Modifier> getGameTextWhileActiveInPlayModifiers(SwccgGame game, final PhysicalCard self) {
        ...
        Filter republicStarships = Filters.and(Filters.your(playerId), Icon.COU, Filters.starship, Filters.at(Filters.and(Filters.system,
                Filters.not(Filters.sameSystemAs(self, Filters.and(Filters.your(playerId), Filters.starship, Filters.not(Icon.COU)))))));
