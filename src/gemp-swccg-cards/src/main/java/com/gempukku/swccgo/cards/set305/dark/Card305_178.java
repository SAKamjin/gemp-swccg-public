 ...
 * Type: Weapon
 * Subtype: Character
 * Title: Apprentice's Lightsaber
 */
public class Card305_178 extends AbstractCharacterWeapon {
    public Card305_178() {
        super(Side.DARK, 1, "Apprentice's Lightsaber", Uniqueness.UNIQUE, ExpansionSet.ABT, Rarity.R);
        setVirtualSuffix(true);
        setLore("Built by apprentices at the Shadow Academy these blades are crude but functional.");
        setGameText("Deploy on an apprentice. May add 1 to Force drain where present. May target a character or creature for free. " +
                    "Draw two destiny. Subtract 1 if targeting a Jedi. Add 1 if targeting a Padawan. Target hit, and its forfeit = 0, if total destiny > defense value.");
        addKeywords(Keyword.LIGHTSABER);
        addIcon(Icon.ABT);
        setMatchingCharacterFilter(Filters.apprentice);
    }

    @Override
    protected Filter getGameTextValidDeployTargetFilter(SwccgGame game, PhysicalCard self, PlayCardOptionId playCardOptionId, boolean asReact) {
        return Filters.and(Filters.your(self), Filters.apprentice);
    }

    @Override
    protected Filter getGameTextValidToUseWeaponFilter(final SwccgGame game, final PhysicalCard self) {
        return Filters.apprentice;
    }
    ...
    protected List<Modifier> getGameTextWhileActiveInPlayModifiers(SwccgGame game, final PhysicalCard self) {
        ...
        modifiers.add(new TotalWeaponDestinyModifier(self, -1, Filters.Jedi));
        return modifiers;
    }
}
