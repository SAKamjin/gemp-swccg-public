 ...
 * Type: Interrupt
 * Subtype: Lost
 * Title: Selika Strikes
 */
public class Card305_105 extends AbstractLostInterrupt {
    public Card305_105() {
        super(Side.DARK, 5, Title.Selika_Strikes, Uniqueness.UNRESTRICTED, ExpansionSet.ABT, Rarity.R);
        setLore("He will fall to the Dark Side...or else!");
        setGameText("If Selika in battle with a Jedi, add one battle destiny (two if Jedi is Turel). OR Use 1 Force to take any lightsaber into hand from Reserve Deck; reshuffle. OR Target a Jedi present with Selika; they duel: Both players draw 2 destiny. Loser (lowest total) is lost.");
        addIcons(Icon.ABT);
    }
    ...
    protected List<PlayInterruptAction> getGameTextTopLevelActions(final String playerId, SwccgGame game, PhysicalCard self) {
        ...
        // Check condition(s)
        if (GameConditions.isDuringBattleWithParticipant(game, Filters.Selika)
                && GameConditions.canAddBattleDestinyDraws(game, self)) {
            if (GameConditions.isDuringBattleWithParticipant(game, Filters.and(Filters.Jedi, Filters.not(Filters.Turel)))) {
            ...
            if (GameConditions.isDuringBattleWithParticipant(game, Filters.and(Filters.Jedi, Filters.Turel))) {
        ...
        GameTextActionId gameTextActionId = GameTextActionId.SELIKA_STRIKES__UPLOAD_LIGHTSABER;
        ...
        if (selika != null) {
            Filter targetFilter = Filters.and(Filters.opponents(self), Filters.Jedi, Filters.presentWith(self, Filters.sameCardId(selika)));
            ...
            if (GameConditions.canTarget(game, self, targetingReason, targetFilter)) {
                ...
                final PlayInterruptAction action = new PlayInterruptAction(game, self, gameTextActionId);
                action.setText("Initiate duel between Selika and a Jedi");
                // Choose target(s)
                action.appendTargeting(
                        new TargetCardOnTableEffect(action, playerId, "Choose Jedi", targetingReason, targetFilter) {
                            @Override
                            protected void cardTargeted(final int targetGroupId, final PhysicalCard targetedCard) {
                                action.addAnimationGroup(selika, targetedCard);
                                // Allow response(s)
                                action.allowResponses("Initiate duel between " + GameUtils.getCardLink(selika) + " and " + GameUtils.getCardLink(targetedCard),
                                        new RespondablePlayCardEffect(action) {
                                            @Override
                                            protected void performActionResults(final Action targetingAction) {
                                                // Perform result(s)
                                                action.appendEffect(
                                                        new DuelEffect(action, selika, targetedCard, new DuelDirections() {
                                                            @Override
